...
import { curriculumStats, levelForXp, passingScoreFor, questionCountFor, trainingBadgeCatalog } from "@/lib/training/gamify"
...
export default function PortalTrainingPage() {
  ...
  const level = levelForXp(progress?.xp ?? 0, stats.maxXp)
  ...
  const pathPercent = curriculum.length
    ...
    : 0
  const badges = trainingBadgeCatalog(stats.moduleCount)
    ...
    <div className="space-y-6 px-4 py-6 lg:px-6 lg:py-8">
      <div>
        ...
        <p className={`mt-1 text-sm ${portalCanvasMuted}`}>
          {stats.moduleCount
            ? `Pass all ${stats.moduleCount} modules and at least ${stats.passingScore}% on each quiz to become a Leo.`
      ...
      <section className="rounded-md bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 text-center sm:grid-cols-4">
          <div className="rounded-md bg-amber-50 p-2">
            <p className="text-lg font-semibold text-amber-800">
              {progress?.xp ?? 0}
              <span className="text-xs font-medium text-neutral-500">/{stats.maxXp || 0}</span>
            </p>
            <p className="text-xs text-neutral-600">Total XP</p>
          </div>
          <div className="rounded-md bg-amber-50 p-2">
            <p className="text-lg font-semibold text-amber-800">{score}%</p>
        ...
        <p className="mt-3 text-xs text-neutral-500">
          {stats.moduleCount} modules · {stats.questionCount} questions · {stats.resourceCount} resources · pass mark {stats.passingScore}%
        </p>
        {completed ? (
      ...
      <section className="rounded-md bg-white p-4 shadow-sm">
        <h2 className="mb-3 font-semibold text-neutral-900">Awards</h2>
      ...
      <section className="space-y-3">
        ...
        {curriculum.length === 0 ? (
          ...
          curriculum.map((module, index) => {
            ...
            const questionCount = questionCountFor(module)
            return (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-2 text-xs font-medium text-amber-700">
                      +{module.xp} XP · {questionCount} question{questionCount === 1 ? "" : "s"}
                ...
                <div className="mt-3">
                  {unlocked ? (
                    <Button asChild className="rounded-md bg-leo-primary text-white">
                      <Link href={`/portal/training/${module.id}`}>
                        {done
                          ? "Review"
                          : latestScore != null && latestScore < passingScoreFor(module.quiz?.passingScore)
                            ? "Retake"
                            : "Continue"}
                      </Link>
                    </Button>
      ...
      <p className={`flex items-center gap-2 text-sm ${portalCanvasMuted}`}>
        ...
        Average quiz score: {score}% · Need {stats.passingScore}% on every published module.
      </p>
